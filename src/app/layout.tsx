...
const clarityScript = `
  ...
  })(window, document, "clarity", "script", "ymektsmzgh");
`;
...
export default function Layout({children}:{children:React.ReactNode}){const schema={"@context":"https://schema.org","@type":["Organization","ProfessionalService"],name:company.name,url:siteUrl,'@id':`${siteUrl}/#organization`,address:{"@type":"PostalAddress",streetAddress:'55 Cartman Street, West Mambalam',addressLocality:'Chennai',postalCode:'600033',addressCountry:'IN'},email:company.email,telephone:company.phone,founder:{"@type":"Person",name:'M. Ubendiran',jobTitle:'Founder & Principal Consultant'}};return <html lang="en"><head><script id="microsoft-clarity" dangerouslySetInnerHTML={{__html:clarityScript}}/></head><body><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><Analytics/><SiteHeader/>{children}<footer className="footer"><div><Link className="brand-link" href="/"><Brand/></Link><p>{company.address}</p></div><nav><a data-analytics="phone_clicked" href={'tel:'+company.phone.replaceAll(' ','')}>{company.phone}</a><a data-analytics="email_clicked" href={'mailto:'+company.email}>Email ↗</a><Link href="/privacy">Privacy</Link></nav></footer></body></html>}
